<template>
    <div class="flex">
      <!-- Sidebar h-[100dvh] --> 
      <aside :class="[ 'fixed top-0 left-0 z-40 h-screen w-72 bg-base-200 text-base-content flex flex-col transition-transform duration-300 ease-in-out', isDrawerOpen ? 'translate-x-0' : '-translate-x-full']">
            <!-- Top Navigation -->
                ...
                <div class="flex items-center space-x-3">
                    <div class="avatar avatar-online">
                        <div class="w-10 rounded-full bg-neutral text-neutral-content flex items-center justify-center">
                            <img :src="authentication?.avatar()" alt="User Avatar" />
                        </div>
                    </div>
        ...
        <div
            ...
            <header class="navbar shadow-md px-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-xs text-base-content/70">Welcome back, {{ user?.firstname + " " + user?.lastname }}</p>
                </div>
...
const user = ref(authentication.data())
...
onMounted(async () => {
    const profile = await authentication.userProfile()
    if (profile) {
        user.value = profile
    }
})
